...
function configDataModifyAfterSaveCallback1(paramsObj){
	var pageid = paramsObj.pageid;
	pageid = pageid.replace(/_add$/,"");
	pageid = pageid.replace(/_detail$/,"");
	parent.searchReportData(pageid,'report1');
	setTimeout(function(){art.dialog.close();}, 600);
	
}

function configDataModifyAfterSaveCallback2(paramsObj){
	var pageid = paramsObj.pageid;
	pageid = pageid.replace(/_add1$/,"");
	pageid = pageid.replace(/_detail$/,"");
	parent.searchReportData(pageid,'report2');
	setTimeout(function(){art.dialog.close();}, 600);
}
function configDataModifyAfterSaveCallback3(paramsObj){
	var pageid = paramsObj.pageid;
	pageid = pageid.replace(/_add2$/,"");
	pageid = pageid.replace(/_detail$/,"");
	parent.searchReportData(pageid,'report1');
	setTimeout(function(){art.dialog.close();}, 600);
}
